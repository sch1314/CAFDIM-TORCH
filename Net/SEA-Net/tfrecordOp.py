...
# def recordGen():
#
#     lst = '/tmp/DataNPZ/TrainProjImgs1e6V60/'
#     cnt = 0
#
...
def recordGen():
    ...
    npz_files = [f for f in os.listdir(lst) if f.endswith('.npz')]

    for fileName in npz_files:
        file_path = os.path.join(lst, fileName)
        ...
        Input = np.float32(data['ldct'])
        Label = np.float32(data['hdct'])
