...
def save_model(net, optimizer, epoch, save_dir, scheduler=None,  best_val_loss=float('inf'), is_best=False):
    '''save model'''
    ...
    if scheduler is None:
        ...
        # 删除 /best/ 目录下已有的文件,即每次最佳后覆盖之前的best
        for filename in os.listdir(best_dir):
            file_path = os.path.join(best_dir, filename)
        ...
        torch.save({
            ...
            os.path.join(best_dir, 'model_at_epoch_best%03d.dat' % (epoch)))
        print(os.path.join(best_dir, 'model_at_epoch_best_%03d.dat' % (epoch)))  # 只要这次epoch是最低loss，就会输出保存并打印
    else:
        torch.save({
            'state_dict': state_dict,
            'optimizer_state_dict': optimizer.state_dict(),
            'lr_scheduler': scheduler.state_dict()},
            os.path.join(save_dir, 'model_at_epoch_%03d.dat' % (epoch)))  # 其它不是最低点的loss的加载点也保存
...
def recon_ops(source_det_distance=1085.6, source_distance=595.0, image_size=512, pixel_spacing=1.4285, det_count=729,
              det_spacing=1.2858, angles=np.linspace(0, 2*np.pi, 720, endpoint=False)):
